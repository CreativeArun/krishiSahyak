...
// ========================================
// SMART ADVISORY FUNCTIONALITY
// ========================================

// Generate AI-powered advisory
async function generateAdvisory() {
    const statusEl = document.getElementById('advisory-status');
    const loadingEl = document.getElementById('advisory-loading');
    const resultsEl = document.getElementById('advisory-results');
    const messageEl = document.getElementById('advisory-message');
    const metricsEl = document.getElementById('advisory-metrics');
    
    // Get current soil and weather data
    const dataset = (typeof districtSoilData !== 'undefined') ? districtSoilData : (typeof window.districtSoilData !== 'undefined' ? window.districtSoilData : undefined);
    const soilData = dataset ? dataset[currentDistrict] : null;
    
    if (!soilData || !lastWeatherData) {
        showToast('Please wait for soil and weather data to load', 'warning');
        return;
    }
    ...
    try {
        ...
        statusEl.className = 'px-2 py-1 text-[10px] rounded bg-yellow-100 dark:bg-yellow-900/40 text-yellow-700 dark:text-yellow-200';
        ...
        messageEl.textContent = data.advisory;
        
        // Display soil metrics
        if (data.soilData) {
            const metricsHTML = Object.entries(data.soilData)
                .slice(0, 6)
                .map(([key, value]) => `
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs bg-indigo-50 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-200 border border-indigo-200 dark:border-indigo-800">
        ...
        statusEl.className = 'px-2 py-1 text-[10px] rounded bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-200';
        ...
        statusEl.className = 'px-2 py-1 text-[10px] rounded bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-200';
        showToast('Failed to generate advisory. Please try again.', 'error');
    }
}

// Initialize Smart Advisory page
window.addEventListener('load', () => {
    const generateBtn = document.getElementById('generate-advisory-btn');
    if (generateBtn) {
        generateBtn.addEventListener('click', generateAdvisory);
    }
});
...
function addMessageToChat(sender, text) {
    ...
    if (sender === 'user') {
        ...
        messageDiv.innerHTML = `
            <div class="bg-green-600 text-white rounded-lg p-3 max-w-xs shadow-sm">
                <p class="text-sm">${escapeHtml(text)}</p>
            </div>
            <div class="w-8 h-8 bg-gray-300 dark:bg-gray-600 rounded-full flex items-center justify-center flex-shrink-0">
                <i data-lucide="user" class="w-4 h-4 text-gray-700 dark:text-gray-200"></i>
            </div>
        `;
    } else {
        ...
        messageDiv.innerHTML = `
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg p-3 max-w-xs shadow-sm">
